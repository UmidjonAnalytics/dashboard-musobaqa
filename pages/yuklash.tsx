...
export default function YuklashPage({ user, phase, forced, supabaseUrl }: Props) {
  ...
  async function handleSubmit() {
    ...
    // Power BI needs a working embed; the live report replaces the
    // screenshot as the thing voters look at.
    if (competition === "powerbi") {
      if (!extractEmbedUrl(embed)) {
        setError(embed.trim() ? uz.embed.invalid : uz.embed.required);
        return;
      }
    }
    ...
    // Excel must have at least one image; Power BI may have none.
    if (competition === "excel" && filled.length < 1) {
    ...
    if (filled.length > 0 && mainCount !== 1) {
    ...
    try {
      ...
      const res = await fetch("/api/submit", {
        ...
        body: JSON.stringify({
          ...
          fileSize: file.size,
          images: imagePayload,
          embed: competition === "powerbi" ? embed.trim() : "",
        }),
      });
      ...
      if (!res.ok) {
        ...
        setError(messageForError(code, detail));
        setBusy(false);
        setProgress("");
        return;
      }
  ...
  return (
    <Shell>
      ...
      <div className="mt-8 space-y-7">
        ...
        {competition === "powerbi" ? (
          <Field label={uz.embed.label} hint={uz.embed.hint}>
            <textarea
              value={embed}
              rows={3}
              maxLength={4000}
              placeholder={uz.embed.placeholder}
              onChange={function (e) {
                setEmbed(e.target.value);
              }}
              className="w-full rounded-xl border border-line bg-white px-4 py-2.5 font-mono text-xs outline-none focus:border-accent"
            />
            {embed.trim() ? (
              extractEmbedUrl(embed) ? (
                ...
                <p className="mt-2 text-sm font-semibold text-warn">{uz.embed.invalid}</p>
              )
            ) : null}
          </Field>
        ) : null}
        ...
        <Field
          label={uz.upload.images}
          hint={
            competition === "powerbi" ? uz.embed.imagesOptional : uz.upload.imagesHint
          }
        >
...
function messageForError(code: string, detail?: string): string {
  ...
  if (code.indexOf("bad_embed") !== -1) return uz.embed.invalid;
  if (code.indexOf("bad_image_count") !== -1) return uz.uploadError.noImages;
  // Anything unexpected shows the database's own words, so a problem can
  // be diagnosed from the screen instead of guessed at.
  if (detail) return uz.uploadError.generic + " [" + detail + "]";
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      ...
      <div className="mt-8 text-center">
        <Link href="/ishlar" className="text-sm text-ink/45 underline underline-offset-4">
          {uz.works.title}
        </Link>
      </div>
    </main>
