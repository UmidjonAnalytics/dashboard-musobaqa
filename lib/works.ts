...
export async function listWorks(competition: Competition): Promise<PublicWork[]> {
  ...
  const { data, error } = await admin
    .from("submissions")
    .select(
      "id, competition, title, description, embed_url, submission_images(image_url, is_main, position)"
    )
...
// Both competitions together, for the mixed Works and Voting listings.
export async function listAllWorks(): Promise<PublicWork[]> {
  const admin = supabaseAdmin();
  ...
  if (error || !data) {
    console.error("listAllWorks failed:", error);
    return [];
  }
