// ============================================================================
// Saytdagi BARCHA o'zbekcha matnlar shu faylda.
// So'zlarni o'zgartirmoqchi bo'lsangiz, faqat shu faylni tahrirlang.
// Kodning boshqa qismiga tegish shart emas.
// ============================================================================

export const uz = {
  siteTitle: "Dashboard Musobaqasi",
  tagline: "Excel va Power BI dashboardlari musobaqasi",

  nav: {
    home: "Bosh sahifa",
    works: "Ishlar",
    upload: "Yuklash",
    vote: "Ovoz berish",
    results: "Natijalar",
    feedback: "Menga bildirilgan fikrlar",
    login: "Kirish",
    logout: "Chiqish",
  },

  home: {
    welcomeBack: "Xush kelibsiz",
    notLoggedIn: "Siz tizimga kirmagansiz.",
    loginHint:
      "Sinf guruhlaridan birining a'zosi bo'lsangiz, quyidagi tugma orqali kiring. Kirmasdan ham saytni ko'rishingiz mumkin.",
    eligible: "Siz dashboard yuklashingiz va ovoz berishingiz mumkin.",
    notEligible:
      "Hisobingiz topildi, lekin siz hozircha hech qanday tasdiqlangan sinf guruhida emassiz. Agar bu xato bo'lsa, o'qituvchingizga murojaat qiling.",
  },

  login: {
    title: "Telegram orqali kirish",
    preparing: "Tayyorlanmoqda...",
    step1: "1-qadam: quyidagi tugmani bosing. Telegram ilovasi ochiladi.",
    step2: "2-qadam: bot yuborgan xabardagi \"Ha, tasdiqlayman\" tugmasini bosing.",
    openTelegram: "Telegramda ochish",
    waiting: "Tasdiqlashingizni kutyapmiz...",
    cancelled: "Kirish bekor qilindi.",
    expired: "Kirish havolasi eskirdi. Qaytadan urinib ko'ring.",
    retry: "Qaytadan urinish",
  },

  bot: {
    askConfirm: "Saytga kirishni tasdiqlaysizmi?",
    confirmYes: "Ha, tasdiqlayman",
    confirmNo: "Bekor qilish",
    confirmed: "Tasdiqlandi. Endi brauzeringizga qayting.",
    cancelled: "Kirish bekor qilindi.",
    toastConfirmed: "Tasdiqlandi!",
    toastCancelled: "Bekor qilindi.",
    toastExpired: "Bu havola endi amal qilmaydi.",
    linkExpired: "Bu kirish havolasi eskirgan. Saytga qaytib, qaytadan urinib ko'ring.",
    noToken: "Saytdagi \"Telegram orqali kirish\" tugmasi orqali kiring.",
  },

  upload: {
    title: "Dashboard yuklash",
    competition: "Musobaqa",
    excel: "Excel",
    powerbi: "Power BI",
    dashTitle: "Ishingiz nomi",
    dashTitleHint: "Bu nom ovoz beruvchilarga ko'rinadi. Ismingizni yozmang.",
    description: "Qisqacha izoh (ixtiyoriy)",
    file: "Dashboard fayli",
    fileHintExcel: "Ruxsat etilgan: .xlsx, .xlsm, .xls -- eng ko'pi 50 MB",
    fileHintPowerbi: "Ruxsat etilgan: .pbix -- eng ko'pi 50 MB",
    images: "Rasmlar (1 tadan 4 tagacha)",
    imagesHint: "Bittasini asosiy rasm sifatida belgilang. Asosiy rasm ro'yxatlarda ko'rinadi.",
    addImage: "Rasm qo'shish",
    addLink: "Havola qo'shish",
    removeImage: "O'chirish",
    mainImage: "Asosiy rasm",
    imageUrlPlaceholder: "Rasm yoki GIF havolasi",
    submit: "Yuborish",
    submitting: "Yuborilmoqda...",
    uploadingFile: "Dashboard fayli yuklanmoqda...",
    uploadingImages: "Rasmlar yuklanmoqda...",
    saving: "Saqlanmoqda...",
    success: "Ishingiz muvaffaqiyatli yuborildi!",
    successHint: "Ovoz berish bosqichi boshlanganda ishingiz ro'yxatda paydo bo'ladi.",
  },

  uploadError: {
    notLoggedIn: "Avval Telegram orqali kiring.",
    notEligible: "Siz dashboard yuklay olmaysiz.",
    notUploadPhase: "Hozir yuklash bosqichi emas.",
    alreadySubmitted: "Siz bu musobaqaga allaqachon ish yuborgansiz. Har bir musobaqaga bittadan ish yuborish mumkin.",
    noFile: "Dashboard faylini tanlang.",
    badExtension: "Fayl turi mos emas.",
    fileTooBig: "Fayl juda katta (eng ko'pi 50 MB).",
    imageTooBig: "Rasm juda katta (eng ko'pi 5 MB).",
    noTitle: "Ishingiz nomini yozing.",
    noImages: "Kamida bitta rasm qo'shing.",
    tooManyImages: "Eng ko'pi 4 ta rasm.",
    noMain: "Bitta asosiy rasmni belgilang.",
    generic: "Xatolik yuz berdi. Qaytadan urinib ko'ring.",
  },

  phase: {
    before: "Yuklash bosqichi hali boshlanmagan.",
    uploadOpen: "Yuklash bosqichi ochiq.",
    votingOpen: "Ovoz berish bosqichi ochiq. Yuklash yopilgan.",
    after: "Musobaqa yakunlandi. G'oliblar e'lon qilindi!",
    testingBanner: "SINOV REJIMI: bosqich qo'lda o'rnatilgan.",
  },

  phaseShort: {
    before: "Tez orada",
    upload: "Yuklash ochiq",
    voting: "Ovoz berish",
    after: "Yakunlandi",
    testing: "SINOV",
  },

  works: {
    title: "Yuborilgan ishlar",
    subtitle: "Barcha ishlarni ko'rish uchun hisob kerak emas.",
    empty: "Hozircha ish yo'q.",
    count: "ta ish",
    open: "Batafsil",
    download: "Faylni yuklab olish",
    votingSoon: "Ovoz berish 11-sentabrda boshlanadi.",
    anonymousNote: "Mualliflar ismlari ovoz berish yakunlanguncha yashirin.",
  },

  work: {
    notFound: "Bunday ish topilmadi.",
    back: "Barcha ishlar",
    description: "Izoh",
    screenshots: "Rasmlar",
  },

  vote: {
    title: "Ovoz berish",
    subtitle: "Har bir ishni uchta mezon bo'yicha 1 dan 5 gacha baholang.",
    design: "Dizayn",
    diagrams: "Diagrammalardan foydalanish",
    problem: "Muammoni yechish",
    suggestion: "Taklifingiz",
    suggestionHint: "Kamida 20 ta belgi. Bu maydon majburiy.",
    suggestionCount: "belgi",
    submit: "Ovoz berish",
    submitting: "Yuborilmoqda...",
    voted: "Ovoz berildi",
    notVoted: "Ovoz berilmagan",
    ownWork: "Bu sizning ishingiz",
    thanks: "Ovozingiz qabul qilindi. Rahmat!",
    yourScore: "Sizning bahoyingiz",
    remaining: "ta ish qoldi",
    allDone: "Barcha ishlarga ovoz berdingiz. Rahmat!",
    listNote: "Ishlar sizga tasodifiy tartibda ko'rsatilyapti.",
    loginFirst: "Ovoz berish uchun Telegram orqali kiring.",
    notVotingPhase: "Hozir ovoz berish bosqichi emas.",
  },

  voteError: {
    tooShort: "Taklif kamida 20 ta belgidan iborat bo'lishi kerak.",
    alreadyVoted: "Siz bu ishga allaqachon ovoz bergansiz.",
    ownWork: "O'z ishingizga ovoz bera olmaysiz.",
    notVotingPhase: "Hozir ovoz berish bosqichi emas.",
    badScores: "Har bir mezonni 1 dan 5 gacha baholang.",
    generic: "Xatolik yuz berdi. Qaytadan urinib ko'ring.",
  },

  results: {
    title: "Natijalar",
    subtitle: "Ballar ovoz berish davomida yangilanib boradi.",
    rank: "O'rin",
    work: "Ish",
    score: "Ball",
    votes: "Ovozlar",
    noVotes: "Ovoz yo'q",
    empty: "Hozircha natija yo'q.",
    countNote: "Ovozlar soni ballning yonida ko'rsatilgan.",
  },

  platform: {
    livePreview: "Jonli ko'rish",
    liveNote: "Bu hisobot Power BI xizmatidan olingan. Uni shu yerda ochib ko'rishingiz mumkin.",
    openInNewTab: "Yangi oynada ochish",
    noScreenshot: "Power BI hisoboti",
  },

  embed: {
    label: "Power BI embed kodi",
    hint: "Power BI xizmatida hisobotni oching: File -> Embed report -> Publish to web (public). Ko'chirilgan kodni shu yerga joylashtiring.",
    placeholder: "<iframe title=... src=\"https://app.powerbi.com/view?r=...\"></iframe>",
    accepted: "Embed havolasi qabul qilindi.",
    invalid: "Bu Power BI havolasiga o'xshamaydi. Faqat powerbi.com havolalari qabul qilinadi.",
    required: "Power BI uchun embed kodi majburiy.",
    imagesOptional: "Power BI uchun rasm ixtiyoriy -- embed asosiy ko'rinish bo'ladi.",
  },

  winner: {
    title: "G'oliblar",
    subtitle: "Musobaqa yakunlandi. Ovoz berish natijalari bo'yicha g'oliblar:",
    badge: "G'OLIB",
    score: "O'rtacha ball",
    votes: "ta ovoz asosida",
    viewWork: "Ishni ko'rish",
    none: "G'olib aniqlanmadi -- hech kimga ovoz berilmagan.",
    congrats: "Tabriklaymiz!",
    allResults: "Barcha natijalar",
  },

  feedback: {
    title: "Ishingiz bo'yicha fikrlar",
    subtitle: "Ovoz berganlar sizning ishingiz haqida quyidagilarni yozishdi.",
    anonymous: "Fikr bildirganlarning ismlari ko'rsatilmaydi.",
    onlyYours: "Bu sahifada faqat o'z ishingizga bildirilgan fikrlar ko'rinadi.",
    noSubmission: "Siz musobaqaga ish yubormagansiz.",
    noVotes: "Bu ishga hech kim ovoz bermagan.",
    yourScore: "Yakuniy ball",
    votesGiven: "ta ovoz",
    rank: "O'rin",
    ofTotal: "tadan",
    criteria: "Mezonlar bo'yicha o'rtacha",
    reviewsTitle: "Yozma fikrlar",
    reviewNumber: "Fikr",
    gaveScore: "Bergan bahosi",
    loginFirst: "Fikrlarni ko'rish uchun Telegram orqali kiring.",
    notYet: "Fikrlar ovoz berish yakunlangach ko'rinadi.",
    thanksNote: "Bu fikrlar keyingi ishingizni yaxshilash uchun. Tanqidni shaxsiy qabul qilmang -- har bir izoh sizga o'sish imkoniyati.",
  },
} as const;
